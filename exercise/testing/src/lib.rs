...
mod test {
    ...
    #[test]
    pub fn test_splish() {
        let test_a = splish(100, 10);
        let test_b = splish(40, 20);
        let test_c = splish(9, 3);

        assert!(test_a == -70, "a is {}", test_a);
        assert!(test_b == 20, "b is {}", test_b);
        assert!(test_c == 0, "c is {}", test_c);
    }
}
